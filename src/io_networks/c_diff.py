...
def build_c_diff(cfg: dict[str, Any]) -> Path:
    ...
    for year in range(year_start, year_end + 1):
        ...
        if len(labels_n) != len(c_diff_n):
            raise ValueError(
                f"Year {year}: label count {len(labels_n)} does not match c_diff_N length {len(c_diff_n)}"
            )

        c_s = pd.Series(c_diff_n, index=labels_n, dtype=float)
        c_n_s = pd.Series(c_n, index=labels_n, dtype=float)
        c_gdp_s = pd.Series(c_gdp_n, index=labels_n, dtype=float)
        ...
        for country in countries:
            ...
            c_diff_country = c_s.reindex(labels_country).to_numpy(dtype=float)
            c_country = c_n_s.reindex(labels_country).to_numpy(dtype=float)
            c_gdp_country = c_gdp_s.reindex(labels_country).to_numpy(dtype=float)
                ...
                weights_rows.append(
                    {
                        ...
                        "c_N": float(c_val),
                        "c_gdp_N": float(c_gdp_val),
                        "c_diff_N": float(c_diff_val),
    ...
    if not c_rows:
        raise ValueError("No c_diff rows were generated. Check blocks/raw inputs and year range.")

    c_df = pd.DataFrame(c_rows).sort_values(["year", "country"]).reset_index(drop=True)
    ...
    c_path = out_dir / "c_diff_by_country_year.parquet"
    ...
    summary_df = (
        c_df.groupby("year", as_index=False)
        .agg(
    ...
    summary_path = out_dir / "c_diff_summary.csv"
    try:
        summary_df.to_csv(summary_path, index=False)
    except PermissionError:
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        summary_path = out_dir / f"c_diff_summary_{timestamp}.csv"
        summary_df.to_csv(summary_path, index=False)
