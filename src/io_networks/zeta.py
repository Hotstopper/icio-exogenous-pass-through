...
def build_zeta(cfg: dict[str, Any]) -> Path:
    ...
    if not blocks_dir.exists():
        raise FileNotFoundError(
            f"Blocks directory not found: {blocks_dir}. Run build-blocks first."
        )
    ...
    out_dir = paths["processed"] / "zeta" / variant
    ...
    for year in range(year_start, year_end + 1):
        ...
        if len(labels_n) != len(chi_n):
            raise ValueError(
                f"Year {year}: label count {len(labels_n)} does not match chi length {len(chi_n)}"
            )

        chi_s = pd.Series(chi_n, index=labels_n, dtype=float)
        countries = sorted(
        ...
        for country in countries:
            ...
            chi_country = chi_s.reindex(labels_country).to_numpy(dtype=float)
            w = weights_norm.to_numpy(dtype=float)

            if np.isnan(chi_country).any():
                zeta_rows.append(
                    {
                        "year": year,
                        "country": country,
            ...
            zeta = float(np.dot(w, chi_country))
            ...
            for label, wr, wn, chi_val in zip(
                labels_country,
    ...
    if not zeta_rows:
        raise ValueError("No zeta rows were generated. Check blocks/raw inputs and year range.")

    zeta_df = pd.DataFrame(zeta_rows).sort_values(["year", "country"]).reset_index(drop=True)
    ...
    summary_path = out_dir / "zeta_summary.csv"
    try:
        summary_df.to_csv(summary_path, index=False)
    except PermissionError:
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        summary_path = out_dir / f"zeta_summary_{timestamp}.csv"
        summary_df.to_csv(summary_path, index=False)
