...
def build_c(cfg: dict[str, Any]) -> Path:
    ...
    out_dir = paths["processed"] / "c" / variant
    ...
    for year in range(year_start, year_end + 1):
        ...
        if len(labels_n) != len(c_n):
            raise ValueError(
                f"Year {year}: label count {len(labels_n)} does not match c_N length {len(c_n)}"
            )

        c_s = pd.Series(c_n, index=labels_n, dtype=float)
        ...
        for country in countries:
            ...
            c_country = c_s.reindex(labels_country).to_numpy(dtype=float)
            w = weights_norm.to_numpy(dtype=float)

            if np.isnan(c_country).any():
                c_rows.append(
                    {
                        "year": year,
                        "country": country,
            ...
            c_scalar = float(np.dot(w, c_country))
    ...
    if not c_rows:
        raise ValueError("No c rows were generated. Check blocks/raw inputs and year range.")
    ...
    c_path = out_dir / "c_by_country_year.parquet"
    ...
    summary_path = out_dir / "c_summary.csv"
    try:
        summary_df.to_csv(summary_path, index=False)
    except PermissionError:
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        summary_path = out_dir / f"c_summary_{timestamp}.csv"
        summary_df.to_csv(summary_path, index=False)
